-- Tutaj znajdują się wszystkie pomysły jakie nie zostały zakończone, lecz można je wprowadzić potencjalnie.
################################################################################
...
begin
  ...
  set czas_pomoc = czas;
    ...
    while iterator1 < ilosc do # przejście po wszystkich godzinach
      call czy_sa_wolne_miejsca_na_stanowisku_ciagle(data, czas_pomoc, id_uslugi, )
      set warunek = if ();
...
################################################################################
# ta procedura liczy czy są wolne miejsca o danej godzinie i czy nie będzie ten nowy zabieg przeszkadzać w przyszłości
delimiter //
